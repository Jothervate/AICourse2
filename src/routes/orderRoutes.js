...
router.post('/', (req, res) => {
  ...
  const createOrder = db.transaction(() => {
    db.prepare(
      `INSERT INTO orders (id, order_no, merchant_trade_no, user_id, recipient_name, recipient_email, recipient_address, total_amount)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(orderId, orderNo, merchantTradeNo, userId, recipientName, recipientEmail, recipientAddress, totalAmount);
  ...
  res.status(201).json({
    data: {
      ...
      payment_url: `/payment/redirect/${order.merchant_trade_no}`
    },
    error: null,
    message: '訂單建立成功'
  });
