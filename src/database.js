...
function initializeDatabase() {
  ...
  migrateOrderStatusForCanceled();
  migrateOrderItemsForeignKeyToOrders();
...
function migrateOrderStatusForCanceled() {
  const table = db.prepare(
    "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'orders'"
  ).get();

  if (!table || table.sql.includes("'canceled'")) return;
  ...
  db.exec(`
    ...
    INSERT INTO orders (
      ...
      total_amount, status, created_at, merchant_trade_no, ecpay_trade_no,
      ecpay_rtn_code, ecpay_rtn_msg, paid_at
    )
    SELECT
      id, order_no, user_id, recipient_name, recipient_email, recipient_address,
      total_amount, status, created_at, merchant_trade_no, ecpay_trade_no,
...
function migrateOrderItemsForeignKeyToOrders() {
  const table = db.prepare(
    "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'order_items'"
  ).get();

  if (!table || !table.sql.includes('orders_old')) return;
